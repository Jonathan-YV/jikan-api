...
async function informacion(url,imagen,informacionAnime) {

    const respuesta = await fetch(url);
    const datos = await respuesta.json();

    const template = `  
                        <h1>${datos.data.title}</h1>
    ...
    const templateImg = `
                            <img src="${datos.data.images.jpg.large_image_url}" alt="">
                        `;
...
informacion(urlID,imagen, informacionAnime);